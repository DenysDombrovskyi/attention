...
df = pd.DataFrame(data, columns=[
    ...
    "APM", "ACPM", "APM (з якістю)", "ACPM (з якістю)"
])
...
if total_input_budget > 0:
    ...
    # Обмеження: бюджети не можуть бути від'ємними
    bounds = [(0, None)] * num_tools
    ...
    if result.success:
        ...
        st.dataframe(comparison_df.style.format({
            "Ваш спліт": ["{:,.0f}", "{:,.2f}", "{:,.2f}", "{:,.2f}", "{:,.2f}"],
            "Оптимізований спліт": ["{:,.0f}", "{:,.2f}", "{:,.2f}", "{:,.2f}", "{:,.2f}"]
        }))
        ...
        st.error("Не вдалося знайти оптимальне рішення. Перевірте, чи всі необхідні дані введені коректно.")
else:
